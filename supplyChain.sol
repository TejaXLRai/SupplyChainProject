...
contract CoffeeSupplyChain is Ownable
{
    ...
    event setUsers(address indexed user, string  name, string  contactNo, string  role, bool isActive, string  profileHash);
  ...
  /*Initialize struct pointer*/
    ...
    function addUser(address _userAddress,
                     ...
                     string memory  _role, 
                     bool _isActive,
                     string memory  _profileHash) public onlyOwner returns(bool){
      ...
      function setUser(address _userAddress,
                     ...
                     string memory  _role, 
                     bool _isActive,
                     string memory  _profileHash) private onlyAuthCaller returns(bool){
  ...
  //      address batchNo = address (keccak256(msg.sender, block.timestamp));
    ...
    function getFarmInspectorData(address batchNo) public  view returns (string  memory coffeeFamily,string memory  typeOfSeed,string memory  fertilizerUsed){
     ...
     function updateFarmInspectorData(address batchNo,
                                    string memory  _coffeeFamily,
                                    string memory  _typeOfSeed,
                                    string  memory _fertilizerUsed) public isValidPerformer(batchNo,'FARM_INSPECTION') returns(bool) {
    ...
    /*set Harvester data*/
    function updateHarvesterData(address batchNo,
                              string  memory _cropVariety,
                              string  memory _temperatureUsed,
                              string memory  _humidity) public isValidPerformer(batchNo,'HARVESTER') returns(bool) {
    ...
    /* perform Export */
   /*set Exporter data*/
    function updateExporterData(address batchNo,
                              uint256 _quantity,    
                              string memory  _destinationAddress,
                              string memory  _shipName,
                              string memory  _shipNo,
                              uint256 _estimateDateTime,
                              uint256 _exporterId) public isValidPerformer(batchNo,'EXPORTER') returns(bool) {
   ...
   /*set Importer data*/
    function updateImporterData(address batchNo,
                              uint256 _quantity, 
                              string memory  _shipName,
                              string  memory _shipNo,
                              ...
                              uint256 _importerId) public isValidPerformer(batchNo,'IMPORTER') returns(bool) {
   ...
   /*set Proccessor data*/
    function updateProcessorData(address batchNo,
                              uint256 _quantity, 
                              ...
                              string  memory _processorAddress) public isValidPerformer(batchNo,'PROCESSOR') returns(bool) {
    ...
    function updateUserForAdmin(address _userAddress, string memory _name, string memory _contactNo, string memory _role, bool _isActive,string memory _profileHash) public onlyOwner returns(bool)
    {
        ...
        /* Call Storage Contract */
        bool status = addUser(_userAddress, _name, _contactNo, _role, _isActive, _profileHash);
    ...
    function userUpdate(address _userAddress,
                     string memory _name, 
                     string memory _contactNo, 
                     string memory _role, 
                     bool _isActive,
               ...
               bool status = setUser(_userAddress, _name, _contactNo, _role, _isActive, _profileHash);
                /*call event*/
        emit setUsers(_userAddress,_name,_contactNo,_role,_isActive,_profileHash);
        emit UserRoleUpdate(_userAddress,_role);
